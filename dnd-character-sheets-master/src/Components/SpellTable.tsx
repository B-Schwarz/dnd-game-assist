    ...
    <div className={classes} style={props.style}>
      {props.showLabels ? (
        <div className='d-and-d-spell-header-labels'>
          <label style={{ width: '20px' }}>{props.german ? 'Zauber Grad' : 'Spell Level'}</label>
          <label style={{ width: '80px' }}>{props.german ? 'Zauberplätze Gesamt' : 'Slots Total'}</label>
          <label style={{ width: 'calc(100% - 100px)' }}>{props.german ? 'Verbrauchte Zauberplätze' : 'Slots Remaining'}</label>
        </div>
      ) : null}
      <div className='d-and-d-spell-header'>
        ...
        {props.level === 0 ? (
          <div className='d-and-d-spell-slots'>
            <label>{props.german ? 'Zaubertricks' : 'Cantrips'}</label>
          </div>
        ) : (
          <div className='d-and-d-spell-slots'>
          ...
          <thead>
            <tr>
              <th style={{ width: '30px', position: 'absolute', left: '-7px' }}>
                {props.german ? 'Vorbereitet' : 'Prepared'}
              </th>
              <th>{props.german ? 'Name des Zaubers' : 'Spell Name'}</th>
            </tr>
          </thead>
        ) : null}
        <tbody>
